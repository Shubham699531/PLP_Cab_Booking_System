...
public class DriverRepoImpl implements DriverRepo {
	...
	public Driver getOneDriver(List<Vehicle> vehicles) {
		...
		//returning first driver from the list of free and verified drivers
		driver = drivers.get(0);
		myLogger.debug("No. of drivers available: " + drivers.size());
	...
	public Driver fetchByEmail(String email) {
		...
		try {
			...
			driver = new Driver();
			//Setting -1 for driverId if exception occurs
	...
	public Booking getBookingDetailsForADriver(int driverId) throws NoBookingFoundException{
		...
		try {
			...
			myLogger.warn("No bookings for driver Id: " + driverId + " Exception thrown: " + e.getMessage() );			throw new NoBookingFoundException("No bookings found for Id: " + driverId);
		}
		return booking;
	}
